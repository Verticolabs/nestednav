$(document).ready(function(){
	window.make_menu = function(){
		...
		var nav_container_selector = '.navbar.navbar-inverse';
	...
	//			var main_item_caret_class = (!caret_dropdown.length) ? 'no-caret' : '';
		...
		function set_height(simple_menu){
			simple_height = simple_menu.outerHeight();
			if ($(window).width() > max_width)
				simple_menu.css('height', 'auto').hide();
			else {
				simple_menu.css('height', 'auto');
		...
		function make_menu(){
			...
			ul.closest(nav_container_selector).prepend(simple_menu);
			setTimeout(function(){
